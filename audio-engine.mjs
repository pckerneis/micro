...
export class AudioEngine {
    ...
    loadPatterns(patterns) {
        ...
        for (const [, patternData] of patterns) {
            ...
            if (Array.isArray(patternData.events) && patternData.events.length > 0) {
                // Event-based pattern (arbitrary step lengths)
                const events = [];
                for (const ev of patternData.events) {
                    const beats = Math.max(0, Number(ev.beats || 0));
                    const durTicks = Math.max(1, Math.round(beats * this.ppq));
                    events.push({ token: ev.token ?? null, durTicks });
                }
                // Apply wrap-carry by extending the last sounding event duration
                const carryBeats = Math.max(0, Number(patternData.wrapCarryBeats || 0));
                const carryTicks = Math.round(carryBeats * this.ppq);
                if (carryTicks > 0) {
                    for (let idx = events.length - 1; idx >= 0; idx--) {
                        if (events[idx].token != null) { events[idx].durTicks += carryTicks; break; }
                    }
                }
                const loopTicks = events.reduce((a, e) => a + (e.durTicks || 0), 0) || 1;
                const pattern = {
                    ...base,
                    events,
                    eventIndex: 0,
            ...
            for (const [patternId, pattern] of this.patterns) {
                if (this.currentTick === pattern.nextTick) {
                    if (Array.isArray(pattern.events) && pattern.events.length > 0) {
                        ...
                        const ev = pattern.events[pattern.eventIndex];
                        const tok = ev.token;
                        const durTicks = Math.max(1, ev.durTicks || 1);
                        let timeSec = this.startTime + pattern.nextTick * this.tickSec;
                        timeSec = Math.max(timeSec, now + 0.005);
                        const durSec = Math.max(0.01, durTicks * this.tickSec);

                        if (tok == null) {
                            // Rest: advance only
                        } else if (Array.isArray(tok) || (tok && typeof tok === 'object' && Array.isArray(tok.chord))) {
                            // Chord
